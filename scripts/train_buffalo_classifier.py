...
Using same architecture as cow classifier for consistency
"""
...
class BuffaloClassifier:
    ...
    def train(self, train_loader, val_loader, num_epochs, save_dir):
        ...
        with open(os.path.join(save_dir, 'classes.json'), 'w') as f:
            json.dump(train_loader.dataset.class_to_idx, f, indent=2)
...
def main():
    print("="*60)
    print("BUFFALO BREED CLASSIFIER TRAINING")
    print("="*60)
    
    DATA_DIR = "data/processed_v2/buffaloes/"
    TRAIN_DIR = os.path.join(DATA_DIR, "train")
    VAL_DIR = os.path.join(DATA_DIR, "val")
    
    if not os.path.exists(TRAIN_DIR):
        print(f"✗ Training data not found at: {TRAIN_DIR}")
        print("Run: python scripts/prepare_buffalo_data.py first")
        return
    ...
    classifier = BuffaloClassifier(
        num_classes=train_dataset.num_classes,
        model_name='efficientnet_b0',
        class_weights=class_weights
    )
    
    SAVE_DIR = "models/classification/buffalo_classifier_v1/"
    ...
    print("\nNext steps:")
    print("1. Evaluate: python scripts\\evaluate_buffalo_model.py")
